...
public class InputReceiver : MonoBehaviour
{
    ...
    [SerializeField] private UnityEvent<MouseButton> onClick = new(); // Event triggered when the object is clicked
    [SerializeField] private UnityEvent onPrimaryClick = new(); // Event triggered when the object is clicked
    [SerializeField] private UnityEvent onSecondaryClick = new(); // Event triggered when the object is clicked
    [SerializeField] private UnityEvent onTertiaryClick = new(); // Event triggered when the object is clicked
    ...
    public UnityEvent<MouseButton> OnClick => onClick; // Public property to access the onClick event
    public UnityEvent OnPrimaryClick => onPrimaryClick; // Public property to access the onPrimaryClick event
    public UnityEvent OnSecondaryClick => onSecondaryClick; // Public property to access the onSecondaryClick event
    public UnityEvent OnTertiaryClick => onTertiaryClick; // Public property to access the onTertiaryClick event
    ...
    public void Click(MouseButton button) // Method to handle click events
    {
        onClick?.Invoke(button); // Invoke the click event if it is not null
        switch (button) // Check which button was clicked
        {
            case MouseButton.Left:
                onPrimaryClick?.Invoke(); // Invoke the primary click event if it is not null
                break;
            case MouseButton.Right:
                onSecondaryClick?.Invoke(); // Invoke the secondary click event if it is not null
                break;
            case MouseButton.Middle:
                onTertiaryClick?.Invoke(); // Invoke the tertiary click event if it is not null
                break;
        }
    }
